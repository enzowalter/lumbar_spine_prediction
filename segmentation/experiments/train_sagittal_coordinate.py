...
class PositionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return images, np.array(data['gt_positions']), np.array(data['original_shape'])
...
def validate(model, loader, criterion, device):
    ...
    for images, gt_positions, gt_shape in tqdm.tqdm(loader, desc="Valid"):
        with torch.no_grad():
            ...
            for level in range(5):
                loss += criterion(predictions[:, level], gt_positions[:, level])
...
def train_epoch(model, loader, criterion, optimizer, device):
    ...
    epoch_loss = 0
    for images, gt_positions, gt_shape in tqdm.tqdm(loader, desc="Training", total=len(loader)):
        optimizer.zero_grad()
...
def train_model_sagittal(input_dir, conditions, description, slice_model_path, model_name):
    ...
    model = LumbarPositionModelSagittal()
...
if __name__ == "__main__":
    m1 = train_model_sagittal(
            ...
            "model_segmentation_st2_coordinate.ts",
        )