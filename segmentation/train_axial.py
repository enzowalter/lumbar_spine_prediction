        ...
        for s_id in series_id:
            ...
            if len(coordinates_dict) == len(LEVELS):
                ...
                for coordinate in coordinates_dict:
                    ...
                    dataset_item['output_mask'] = np.zeros(MASK_SIZE)
...
class SegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return images, torch.tensor(data['output_mask']).float().unsqueeze(0), torch.tensor(data['gt_positions'])
...
def train_model_axial(input_dir, condition, description, model_name):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
if __name__ == "__main__":
    train_model_axial(
                ...
                "Axial T2",
                "model_segmentation_ax_right.ts",
            )
    train_model_axial(
                "../",
                "Left Subarticular Stenosis",
                "Axial T2",
                "model_segmentation_ax_left.ts",
            )
