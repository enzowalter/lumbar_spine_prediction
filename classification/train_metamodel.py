...
def validate(model_features, metamodel, loader, criterion, device):
    metamodel.eval()
    model_features.eval()
    ...
    with torch.no_grad():
        for images, labels_gt in tqdm.tqdm(loader, desc="Valid"):
            ...
            features = model_features.forward_encoders(images.to(device))
            predictions = metamodel(features)
            
            i = torch.argmax(predictions, dim = 1)
            matrix[labels_gt.item(), i.item()] += 1

            loss = criterion(predictions, labels_gt)
            classification_loss_sum += loss.item()

            log_loss_batch = calculate_log_loss(predictions, labels_gt, weights)
...
def train_epoch(model_features, metamodel, loader, criterion, optimizer, device):
    metamodel.train()
    epoch_loss = 0
...
def train_metamodel(input_dir, model_name, crop_description, crop_condition, label_condition, crop_size, image_resize, pretrained_path):
    ...
    metamodel = MetaModelClassifier(feature_size=256, seq_lenght=5, n_classes=3)
    ...
    for epoch in range(10):
        loss_train = train_epoch(feature_model, metamodel, train_loader, criterion, optimizer, device)
        metrics = validate(feature_model, metamodel, valid_loader, criterion, device)
        print("Epoch", epoch, "train_loss=", loss_train, "metrics=", metrics)
        if metrics['loss'] < best:
            print("New best model !", model_name)
            best = metrics["loss"]
            torch.save(metamodel.state_dict(), model_name)
        scheduler.step()
        print('-' * 50)
    return best

if __name__ == "__main__":
    best_logloss1 = train_metamodel(
                    input_dir="../../REFAIT",
                    model_name="classification_spinal_canal_stenosis_metamodel.pth",
