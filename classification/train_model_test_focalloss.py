...
def train_submodel_test_slice(input_dir, model_name, crop_description, crop_condition, label_condition, crop_size, image_resize, slice_offset):
    ...
    model = FoldModelClassifier(
        n_classes=3,
        n_fold_classifier=5,
        backbones=['mobilenetv3_small_100.lamb_in1k', 'xception', 'densenet161.tv_in1k', 'resnet26t.ra2_in1k', 'ecaresnet26t.ra2_in1k'],
        features_size=256,
    )
    ...
    criterion = FocalLoss(weights=torch.tensor([1/7, 2/7, 4/7]).to(device))
    best = 123456
