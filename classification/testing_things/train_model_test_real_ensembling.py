...
def validate_ensembling(models, loader, criterion, device):
    ...
    with torch.no_grad():
        for images, labels_gt in tqdm.tqdm(loader, desc="Valid"):
            ...
            for model in models:
                model = model.eval()
                final_output += (model.forward_fold(images.to(device), mode="valid") / len(models))
...
def train_submodel_ensembling(input_dir, model_name, crop_description, crop_condition, label_condition, crop_size, image_resize, slice_offset):
    ...
    for k in range(5):
        ...
        for epoch in range(8):
            ...
            if metrics['logloss'] < best:
                ...
                torch.save(model.state_dict(), '__tmp__.pth')
            scheduler.step()
        ...
        model.load_state_dict(torch.load('__tmp__.pth'))
        models.append(model.to(device))
    
    print("TEST ALL MODELS")
    metrics = validate_ensembling(models, valid_loader, criterion, device)
    print(metrics)
    return metrics

if __name__ == "__main__":
        train_submodel_ensembling(
            input_dir="../../REFAIT",
